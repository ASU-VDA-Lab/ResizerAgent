...
SelectorContext fields:
    plan_results      list[dict]   — the iteration's plan row from iteration_summary.csv
    best_plan         str|None     — name of the iteration's plan if successful (e.g. "plan1")
...
def _resolve_seed_wns(
    ...
    # Iter > 1: seed is from a prior iteration's plan (after-GR, in ps).
...
# ---------------------------------------------------------------------------
# plan-result selection (single plan per iteration)
# ---------------------------------------------------------------------------
...
def _select_best_plan(
    ...
    """Return (plan_name, plan_wns, plan_metrics, regressed).

    With one plan per iteration this picks the successful row if present.
    Marks the iteration as regressed when its WNS is worse than the seed.
    """
...
def _format_prompt_text(
    ...
    if best_plan and best_wns is not None:
        ...
        lines.append(f"CHOSEN PLAN: {best_plan} (WNS={best_wns:.1f} ps"
                     + (f", {delta_str}" if delta_str else "") + f"){reg_note}")
    else:
        lines.append("CHOSEN PLAN: none (plan failed or regressed)")
    lines.append("")
    ...
    d_str = (f"|ΔWNS|={abs(delta_wns_this_iter):.1f} ps this iter"
             if delta_wns_this_iter is not None else "plan did not succeed this iter")
    ...
    if notable:
        ...
        lines.append("PREDICTION CALIBRATION: plan performed as predicted")
...
def build_context(
    ...
    if successful and ctx.seed_wns is not None:
        ...
        all_regressed = True  # plan failed → treat as regressed for plateau purposes
