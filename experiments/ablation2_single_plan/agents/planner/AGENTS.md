...
`default` (ORFS repair_timing + GR, your budget reference), `plan` (your plan + GR).
...
**Both signals present, neither dominant** — choose the side with higher
leverage from your headroom analysis: if WNS gap > recoverable headroom, pick
TNS-broad; if WNS is within reach, pick WNS-surgical. Surface the rejected
alternative in `rationale` so the Selector can recommend it for the next
iteration.
...
## Step 6 — Commit to one plan

Each iteration produces a single execution plan (`plan_count = 1`, `plans`
has exactly one element). Pick the plan whose analytical conclusion has the
highest confidence of moving WNS or TNS. Concrete rules:

- Do not "cover both" — choose either WNS-surgical or TNS-broad based on the
  headroom analysis from Step 2, not as a hedge.
- Mention rejected candidate strategies in `rationale` so the Selector can
  recommend them in the next iteration's guidance.
- Plan-type variety still matters: across iterations you can alternate
  between `sequence`, `staged`, and `eco` as the data dictates.
- Do not encode multiple ideas inside one plan by chaining unrelated moves;
  keep the plan focused on one mechanism.
...
The `PRIOR ITERATION PLAN RESULTS` section shows what the prior iteration's plan
actually achieved: sequence used, Selector rating (A/B/C/F), ΔWNS, ΔTNS, area_ratio,
and why it was rated that way. This is concrete evidence about what worked and what
didn't.

Read it asking:
- Did the sequence produce an efficient improvement (rated A/B) or a wasteful one (rated C/F)?
- What was the actual ΔTNS — did a broad plan move the violation count?
- Did TNS improve meaningfully, or was the gain penalized by area cost?
- Across the last few iterations, is a pattern emerging — e.g., buffer-heavy plans
  consistently rated C due to area overhead without commensurate WNS gain?
...
estimate was. If you consistently over-predicted WNS improvement by 2–3×, the most
likely cause is that you underestimated ceiling stages or did not account for path
competition. Adjust your estimate this iteration accordingly.
...
**Plan body**: `reasoning` = one sentence per move citing the specific stage or cell
