...
- Tell the Planner to pivot to surgical WNS-focused plans (low repair_tns, staged) targeting the specific stuck endpoint.
...
**Case 4 — Both stuck**: true plateau. Hard — the design may have a structural bottleneck that neither broad nor surgical `repair_timing` can address. This is backtrack territory.
...
- Area > 1.15 with WNS stuck: overhead is self-defeating — net capacitance increase hurts neighboring paths. Require `unbuffer`-first or surgical staged plans.
...
- Recommend: low `repair_tns` (0–20), surgical staged plans on the specific bottleneck
