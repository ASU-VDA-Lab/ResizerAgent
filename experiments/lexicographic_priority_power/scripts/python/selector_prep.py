...
def _resolve_seed_wns(
    ...
    Priority order:
      ...
      2. Iteration 1: start/seed_metrics.csv (preferred), else design_dir/base/base_metrics.csv.

    Unit convention: both seed_metrics.csv and base_metrics.csv carry WNS already in ps
    because the writer goes through _collect_metrics (run.py), which applies the active
    PDK's time_unit_ps factor before writing. The earlier "if abs(wns) < 10 then ×1000"
    heuristic was wrong — it tripped on designs with small in-spec WNS (e.g. gcd at 300 ps
    ...
    if iterN == 1:
        ...
        if base_row:
            return _safe_float(base_row.get("wns"))
        return None
    ...
    # Fallback: seed_metrics.csv (also in ps per the unit convention above).
    seed_csv = iter_dir / "start" / "seed_metrics.csv"
    row = _read_csv_first_row(seed_csv)
    if row is not None:
        return _safe_float(row.get("wns"))
    return None
...
def _resolve_seed_tns(
    ...
    Unit convention: both seed_metrics.csv and base_metrics.csv carry TNS already in ps
    via _collect_metrics. The earlier "if abs(tns) < 100 then ×1000" auto-scaling was a
    magnitude heuristic that misfired on small designs and is removed for the same
    reasons documented in _resolve_seed_wns.
    """
    ...
    if iterN == 1:
        ...
        base_row = _read_csv_first_row(design_dir / "base" / "base_metrics.csv")
        if base_row:
            return _safe_float(base_row.get("tns"))
        return None
...
def _select_best_plan(
    ...
    POWER-PRIORITY selection (ablation4):
      - Only successful plans are candidates.
      - WNS guards (BOTH must hold):
          (a) WNS_BAND  — within 10 ps of the best WNS in this iteration.
          (b) WNS_HARD_FLOOR — within 10 ps of seed_wns (no catastrophic
              regression even when every plan regressed).
        The stricter of the two bounds wins. If both bounds reject every plan
        (all plans worse than seed_wns - WNS_HARD_FLOOR), fall back to the
        least-bad WNS plan so the iteration still produces an output.
      - Among candidates: LOWEST power_mw wins. The "tied on power" bucket is
        defined relatively (POWER_TIE_FRAC × min_power) so the threshold
        scales across designs (gcd 2 mW vs aes 570 mW).
      - Tiebreakers inside the power-tied bucket: higher WNS, higher TNS,
        smaller area_um2.
      - If every candidate has no power data, fall back to WNS-first ordering
        so the selector doesn't crash.
      - regressed=True if the picked plan's WNS is worse than seed_wns.
    """
    WNS_BAND        = 10.0    # ps; tolerance vs the best-WNS plan in this iter
    WNS_HARD_FLOOR  = 10.0    # ps; tolerance vs seed_wns (no catastrophic loss)
    POWER_TIE_FRAC  = 0.01    # 1% of min in-band power treated as tied
    ...
    band_floor = best_wns_val - WNS_BAND
    seed_floor = (seed_wns - WNS_HARD_FLOOR) if seed_wns is not None else float("-inf")
    wns_floor = max(band_floor, seed_floor)
    candidates = [p for p in successful if p["wns"] >= wns_floor]
    if not candidates:
        # Every plan blew past the hard floor — keep the least-bad one so
        # the orchestrator still has something to promote (regressed=True).
        candidates = [max(successful, key=lambda p: p["wns"])]

    def pw(p):
        v = p.get("power_mw")
        return v if v is not None else float("inf")

    min_power = min(pw(p) for p in candidates)
    if min_power == float("inf"):
        # No power data anywhere — degrade to WNS-first selection on the
        # candidate set instead of crashing on an empty tied list.
        tied = list(candidates)
    else:
        tol = max(min_power * POWER_TIE_FRAC, 1e-6)
        tied = [p for p in candidates if abs(pw(p) - min_power) <= tol]
    tied.sort(key=lambda p: (
        -(p.get("wns") if p.get("wns") is not None else float("-inf")),
        -(p.get("tns") if p.get("tns") is not None else float("-inf")),
        p.get("area_um2") if p.get("area_um2") is not None else float("inf"),
    ))
    best = tied[0]
...
def _format_prompt_text(
    ...
    for pr in plan_results:
        ...
        power = pr.get("power_mw")   # raw value is OpenROAD report_power Total (Watts)
        area  = pr.get("area_um2")
        ...
        # Match the default_metrics.csv column header + unit so the LLM can
        # compute power_ratio = plan_power / default_power directly. The raw
        # value is in Watts despite the legacy "power_mw" field name; both
        # default and per-plan use that same misnamed field, so ratios are
        # consistent (and ratios are unit-invariant regardless).
        power_str = f"power_mw={power:.3e}" if power is not None else "power_mw=N/A"
        area_str = f"area_um2={area:.0f}" if area is not None else "area_um2=N/A"
        ...
        if status == "success":
            ...
            line += (f" | WNS={wns_str} | {delta_str}{tns_part}{viol_part}"
                     f" | {power_str} | {area_str} | {status}{pred_note}")
        else:
            line += f" | {status}"
