...
## Objective priority — POWER FIRST in this configuration

**Power is the highest-weight objective.** You must still solve WNS and TNS — the
Selector will reject any plan that regresses WNS by more than ~10 ps from seed —
but among plans that meet timing, the Selector explicitly prefers the one with
the lowest power. A plan that holds WNS flat while cutting power 3 % beats a plan
that gains a few ps of WNS while burning more power.

This reshapes how you choose moves. **Important:** `repair_timing -sequence`
moves don't take a direction knob — the resizer picks the direction (faster
cell, more buffers) based on slack. So when this table says a sequence move
"reduces power," it means the move's expected effect *given the slack
landscape* — not a guaranteed direction. Direction-controlled power wins
(e.g., `vt_swap` toward higher Vt) require an **ECO plan** that names the
specific instance and target.

**Sequence moves — what direction the resizer actually picks:**

| Move | Effect in a sequence | Power direction | When to lead with it |
|------|----------------------|-----------------|----------------------|
| `unbuffer` | Removes redundant buffers on slack-positive nets | **Reduces** dynamic power | Long buffer chains (≥ 3 BUFx* in a row); over-buffered launch cones |
| `sizedown` | Downsizes cells with slack headroom | **Reduces** dynamic + leakage | Cells over-driving their load (slew < 5 ps, small cap) |
| `swap` | Functionally equivalent gate swap | Roughly neutral | Edge-case timing reshuffle |
| `sizeup_match` | Equal-footprint upsize on critical cells | Roughly neutral | Drop-in WNS fix without bloat |
| `vt_swap` | Resizer picks faster Vt on critical paths | **INCREASES leakage** on those paths | Only when WNS forces it — this is NOT a sequence-mode power-reducer |
| `sizeup` | Upsizes critical cells | **Increases** dynamic + leakage | Stages where the slack analysis shows it's the only lever |
| `buffer` | Inserts buffers on high-cap nets | **Increases** dynamic power | High-cap nets where the driver is already maxed |
| `clone` | Splits high-fanout drivers | **Increases** power and area | High-fanout shared drivers with no other option |

**ECO plans — where you have direction control (use these for real power wins):**

| ECO action | Power direction | Use case |
|------------|-----------------|----------|
| `vt_swap` to higher Vt (SL → L → R) on a named instance | **Reduces** leakage | Slack-positive cells off the critical path, especially SLVT cells leaking heavily |
| `resize` to smaller drive on a named instance | **Reduces** dynamic + leakage | Overdriven cell with confirmed slack headroom |
| `delete_buffer` on a named buffer | **Reduces** dynamic | Redundant repeater in a long chain |

**Plan-design rule of thumb:**
- Lead `sequence` plans with `unbuffer` / `sizedown` whenever the WNS analysis
  allows — these are the only sequence moves the resizer will actually steer
  toward less power.
- For higher-Vt swaps and instance-pinned shrinks, use an **ECO plan** with
  explicit actions. A sequence containing `vt_swap` does NOT lower power on
  failing paths — it does the opposite.
- Reserve `sizeup` / `buffer` / `clone` for paths where the analysis shows they
  are the ONLY lever. When you use them, justify the power cost in `rationale`
  and quantify the expected `Δpower` in `expected_outcome` (not just `ΔWNS`
  and `ΔTNS`).
