...
def run_reporter(agent: str, design: str, iterN: int,
    ...
    # --- Parse GR metrics from the relevant log (iter1: default; N≥2: winning plan log) ---
    gr_summary = ""
    try:
        sys.path.insert(0, str(SCRIPTS_PY))
            ...
            if best_plan_f.exists():
                for ln in best_plan_f.read_text().splitlines():
                    if ln.startswith("plan="):
                        ...
                        break
            gr_log = (iter_dir(agent, design, source_iter) / winner / f"{winner}.log") \
                     if winner else None
            gr_label = (f"Seed plan ({winner}) global route from Iteration {source_iter}:"
                        if winner else "")
    ...
    if iterN == 1:
        ...
        for k in range(1, iterN):
            ...
            summary_rows.append(f"  Iter{k}: winner={best_name} WNS={best_wns} TNS={best_tns}")
        ...
        for k in range(start_k, iterN):
            ...
            # Seed WNS for iter k = best WNS from iter k-1 (or default if k=1)
            ...
            detail_rows.append(f"  Iter{k} plans tried (seed WNS={seed_str} ps):")
            for row in plan_results:
                ...
                marker = " ← WINNER" if pname == winner_name else ""
...
Recent plan attempts (last 3 iterations — see what has been tried and what failed):
...
def _collect_metrics(plan_dir: pathlib.Path, stage_tag: str) -> Dict:
    ...
    # NOTE on the `power_mw` field: the column was named in mW but
    # _parse_power_file returns the OpenROAD report_power Total column in W.
    # We intentionally keep that raw W value here to preserve aggregate-CSV
    # compatibility with the control runs (plot scripts and generate_report
    # read this column as power_W). The power-priority selector_prep relies on
    # a RELATIVE tie threshold (POWER_TIE_FRAC = 1% of min), so the math is
    # unit-invariant — converting here would silently break the comparison
    # CSVs without changing selection behavior.
...
def run_workers(agent: str, design: str, iteration: int,
    ...
    """Run all plan directories in parallel via Docker. Returns 0 if all OK, 1 if any failed."""
    ...
    log(f"Workers: running {targets} in parallel (Iteration {iteration})")
...
def _build_selector_file_context(agent: str, design: str, iteration: int,
    ...
    - Current iteration best plan worst paths
    - Prior iteration best plan worst paths (N >= 2)
    ...
    if cur_paths.exists():
        parts.append(f"=== WORST PATHS (Iteration {iteration} best plan) ===\n"
...
# ===========================================================================
# SECTION 7 — Promote best plan artifacts
# ===========================================================================
...
def update_best_solutions(agent: str, design: str, iteration: int, decision: dict) -> None:
    ...
    out = {
        ...
        "note": (
            ...
            "All successful plans from every iteration compete — not just selector winners. "
...
def run_llm_iterations(agent: str, design: str, claude: str,
    ...
    for iteration in range(start_iteration, max_iterations + 1):
        ...
        try:
            ...
            log(f"Planner committed: {plan_count} plan(s).")

            # 3. Executor — pure Python generates run_plan.tcl for every plan.
            # LLM executor only kicks in at step 4b if a plan fails in Docker.
            ...
            if _executor_errs:
                for w in _executor_errs:
                    log(f"  [WARN] {w}")
            log(f"  Executor: TCL generated for valid plan(s) (of {plan_count} requested).")
            ...
            if summary_csv.exists():
                ...
                if tcl_failures:
                    ...
                    # Re-run all plans that got fixed (deterministic or LLM)
