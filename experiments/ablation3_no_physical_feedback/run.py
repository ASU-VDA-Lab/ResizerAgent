...
def prepare_default_artifacts(design: str, agent: str) -> None:
    ...
    at-placement save → reports. All inside run_default.tcl.

    Requires prepare_base_artifacts to have been run (base_cts.odb must exist).
    The saved default.odb is the at-placement state — next iteration's seed
    and the backend's starting point. No GR runs in this stage; GR happens
    in the backend (ORFS `make route finish`).
    """
    ddir        = design_dir(agent, design)
    base_dir    = ddir / "base"
...
def _build_prompt(
    ...
    neighbor_netlist: str, cell_catalog: str,
    clock_summary: str = "",
    pdk_constraints: str = "",
) -> str:
...
def run_reporter(agent: str, design: str, iterN: int,
    ...
    log("[1/6] Parsing base + default metrics")
    ...
    log(f"[4/6] Building compact neighbor tables for top {neighbor_paths} endpoints")
    neighbor_netlist = _build_compact_neighbors(sections, netlist_file, top_n=neighbor_paths)

    log("[5/6] Building cell catalog from XML")
    cell_catalog = _build_cell_catalog_from_paths(sections)

    # --- Assemble prompt ---
    log("[6/6] Assembling prompt")
