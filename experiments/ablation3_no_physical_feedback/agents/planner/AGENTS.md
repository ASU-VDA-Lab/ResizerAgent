...
All WNS/TNS/area/power numbers are measured at **placement parasitics** using
`estimate_parasitics -placement`. Stages: `base_cts` (no repair),
`default` (ORFS repair_timing, your budget reference), `plan<i>` (your plan). Global
route is not run inside the loop; ORFS handles GR + detail route in the backend
stage after the loop concludes.
...
No physical / spatial / routing information is provided in this configuration — there
is no placement report and no global-route summary. Reason about timing purely from the
electrical evidence above; do not speculate about cell locations, local density, free
slots, congestion, or layer overflow.
...
- If `net_dly > 0.2 × cell_dly`: wire RC is a factor. Check `cap` — high cap (> 3 fF)
  means high fanout. Buffering splits the load.
...
- If cap is high on many paths → `buffer` or `clone` first.
...
- Good: `"HAxp5 _596_ (52 ps, ceiling — catalog shows single drive strength, already SL).
  ...
  Max recoverable: ~6 ps from INVx1 sizeup + vt_swap. WNS gap 47 ps — path cannot
  close; target TNS breadth."`
  ...
  "plans": [
    ...
    },
    {
      ...
      "reasoning": [
        ...
        "clone: neighbor netlist shows OA211x2_SL _331_ drives fanout-6 net _108_ (cap=5.7fF) feeding 6 of the 12 worst endpoints — clone splits load, reduces cap per driver from 5.7→2.8fF"
      ],
