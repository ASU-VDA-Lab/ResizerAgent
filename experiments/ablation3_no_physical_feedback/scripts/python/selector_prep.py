...
def _resolve_seed_wns(
    ...
    Priority order:
      1. Prior iteration's best_plan.txt wns= field (ps, at-placement) — most reliable for iter > 1.
      ...
      2. Iteration 1: design_dir/base/base_metrics.csv wns column (ps, at-placement).
    ...
    All other WNS values in the LLM loop are in ps (placement parasitics, post-repair).
    """
    llm_root = iter_dir.parent  # llm_iterations/

    if iterN == 1:
        # Iter 1 seeds from base CTS — base_metrics.csv stores pre-RT WNS at
        # placement parasitics, in ns; convert to ps below.
    ...
    # Iter > 1: seed is from a prior iteration's best plan (at-placement, in ps).
