...
The Planner emits one plan per iteration. Rate that plan on absolute progress
against the seed and use the cross-iteration trajectory as your secondary signal.
When recommending the next iteration's strategy, name a single side
(WNS-surgical OR TNS-broad) and name the side to fall back to if it stalls.
...
- **PLAN RESULTS** — the plan's WNS, TNS, ΔWNS, ΔTNS, area, violation count, prediction accuracy
- **CHOSEN PLAN** — the iteration's plan (Python records its metrics for promotion)
...
- **PREDICTION CALIBRATION** — Planner expected vs actual ΔWNS
- **DEFAULT METRICS** — WNS, TNS, area, power after ORFS default repair_timing
- **WORST PATHS** — current and prior iteration worst paths (pre-read)
- **VIOLATING ENDPOINTS** — current iteration's violating endpoints
...
- The plan failed → `decision: "stop"`, `stop_reason: "all_plans_failed"`. Done.
- Plan WNS ≥ 0 → `decision: "stop"`, `stop_reason: "closure_achieved"`. Done.
...
### 2a. Assess the plan on WNS AND TNS

For the plan that ran, compute:
...
- Tell the Planner to pivot to a surgical WNS-focused plan (low repair_tns, ECO, staged) targeting the specific stuck endpoint.
- Recommend ONE for the next iteration (TNS-broad OR WNS-surgical) and name the
  fallback for the iteration after.
...
- Recommend: low `repair_tns` (0–20), ECO or staged plan on specific bottleneck
...
**Tell Planner which side to take** when WNS and TNS are both meaningful:
- Pick the side with higher leverage for *this* iteration and name the other as
  the fallback for the iteration after.
...
Python records the iteration's plan and its metrics. There is no override —
the plan either ran (and its metrics are the iteration's metrics) or it
failed (and the iteration is treated as regressed).

If the plan's area_ratio > 1.15 with a small ΔWNS gain, flag the area cost
in `per_plan_ratings`; the Planner can pivot to `unbuffer`-first next
iteration. If the plan reused a sequence that has stalled 3+ consecutive
iterations with < 3 ps progress, call that out in `guidance` and recommend
a different structural angle for the next iteration.
...
| Plan failed | `stop` / `all_plans_failed` |
...
| Plan regressed, similar path structure | `retry` — same seed, new Planner angle |
| Plan regressed, different path structure | `accept_regression` (≤ 5 ps budget) |
...
```json
{
  ...
  "strategy_note": {
    ...
    "guidance": [
      "Iter1 plan WNS=-33.1ps (Δ+14.6ps) TNS=-3200ps (Δ+1400ps) area_ratio=1.03. continue. Broad vt_swap+sizeup improved TNS significantly; WNS advancing.",
...
Include ΔWNS, ΔTNS, and area_ratio in the note. State whether TNS improved or worsened — this is the breadth signal.

### `guidance`

First entry: summary line `"Iter<N> plan: <plan> WNS=<X>ps (Δ<Y>ps) TNS=<Z>ps (Δ<W>ps) area_ratio=<R>. <decision>. <key observation>."`
