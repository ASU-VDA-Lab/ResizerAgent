"""Pure-Python Executor: validate Planner decision, generate the run_plan.tcl file.

Reads planner_decision.json from the iteration directory, validates the
single plan it contains, and emits run_plan.tcl into the plan1/ subdir.
...
def validate_decision(decision: dict) -> List[str]:
    ...
    if not plans:
        ...
        return errs
    if len(plans) != 1:
        errs.append(f"plans must contain exactly 1 entry, got {len(plans)}")
        return errs
...
def build_plans(iter_dir: pathlib.Path) -> Tuple[bool, List[str]]:
    """Read planner_decision.json, validate, write run_plan.tcl for the iteration's plan.

    Returns False when the JSON is structurally broken or the plan fails
    validation (nothing left to run).
    """
