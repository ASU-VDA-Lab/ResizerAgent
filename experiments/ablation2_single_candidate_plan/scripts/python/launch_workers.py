#!/usr/bin/env python3
"""
launch_workers.py — plan worker launcher for the agentic timing-closure loop.

Runs the iteration's plan inside a Docker/OpenROAD process. OpenROAD output goes
to the plan's log file. Classifies errors from the log and writes structured
metrics.

Does NOT promote the plan or alter the loop control. That is the Selector
agent's job. After this script completes, invoke the Selector agent to
evaluate the result and decide whether to continue or stop the loop.

Usage:
    # Run the iteration's plan
    python3 scripts/python/launch_workers.py \\
        --design aes --agent claude --iteration 10 --plan-count 1

    # Rerun the plan after an Executor TCL fix
    python3 scripts/python/launch_workers.py \\
        --design aes --agent claude --iteration 10 --plan-count 1 \\
        --plans plan1

Exit codes:
    0  — the plan succeeded
    1  — the plan failed (check plan_metrics.csv)
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument("--plan-count", required=True, type=int,
                    help="Number of plans in this iteration (always 1)")
    ap.add_argument("--plans",      nargs="+", metavar="planN",
                    help="Run only the named plan directory (e.g. --plans plan1). "
                         "Default: run the iteration's plan.")
...
def main() -> int:
    ...
    # Run the plan (uses ThreadPoolExecutor for symmetry with the orchestrator)
    results: List[Dict] = []
    ...
    print("Invoke the Selector agent to evaluate the result and steer the loop.")

    # Return non-zero if the plan failed
    any_failed = any(r["status"] != "success" for r in results)
    return 1 if any_failed else 0


def _print_result_line(result: Dict) -> None:
    """Print a status line for the plan. This is the only screen output."""
    p = result["plan"]
    s = result["status"]
    if s == "success":
...
def _write_iteration_summary(iter_dir: pathlib.Path, results: List[Dict]) -> None:
    """Write iteration_summary.csv with the iteration's plan result."""
    fields = [
        "plan", "status", "error_type", "first_error",
        "wns", "tns", "area_um2", "util_percent",
        "power_mw", "violating_endpoints", "log",
