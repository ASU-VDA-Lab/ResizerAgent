...
if {![info exists generate_design_artifacts_skip_load] || !$generate_design_artifacts_skip_load} {
    ...
    read_sdc $sdc_file
    source $setrc_tcl
    # Standalone path: assumes caller passes an after-GR ODB (default.odb,
    # plan<i>/output.odb). For raw post-CTS ODBs with no GR, use
    # generate_base_artifacts.tcl instead (it does placement parasitics).
